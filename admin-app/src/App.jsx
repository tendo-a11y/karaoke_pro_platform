...
// Один QR-код на стол (аудит "Управление клубами": старый venue_qr_code
// генерировал один QR на весь клуб — deep-link в Telegram, гость сам
// выбирал стол внутри бота; Guest App принимает table_no только из URL,
// экрана выбора стола внутри самого приложения нет — см. согласованный
// план блока). Рендерим QR как data URL прямо в браузере (qrcode, без
// обращения к бэкенду за картинкой — он отдаёт только ссылки).
function QrCodeImage({ url, tableNo }) {
  ...
  return (
    <div className="qr-card">
      <div className="qr-card__label">Стол {tableNo}</div>
      {dataUrl ? <img src={dataUrl} alt={`QR стол ${tableNo}`} /> : <p className="empty-hint">Генерация…</p>}
...
function ClubDetail({ token, clubId, isSuperAdmin, onClubChanged, onBack }) {
  ...
  return (
    <div className="panel club-detail">
      ...
      <h3>QR-коды столов</h3>
      {!showQr && <button type="button" className="link-btn" onClick={handleShowQr}>Показать QR-коды</button>}
      {qrError && <div className="banner banner--error">{qrError}</div>}
      {showQr && qrData && (
        qrData.links.length === 0 ? (
          <p className="empty-hint">У клуба не указано число столов (table_count) — нечего генерировать.</p>
        ) : (
          <div className="qr-grid">
            {qrData.links.map((link) => (
              <QrCodeImage key={link.table_no} url={link.url} tableNo={link.table_no} />
            ))}
          </div>
        )
      )}
