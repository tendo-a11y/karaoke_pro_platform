...
отличается (права правообладателя). Поэтому _search_itunes перебирает
несколько магазинов по очереди (см. её докстринг) — это и есть тот
эффект "искать по всему Apple Music", который отдельно попросили при
живой проверке (случай с румынской колядкой "Galbenă Gutuie" — была в
магазинах Британии/США, но не в молдавском).
...
def _search_itunes(query: str, limit: int) -> list[dict]:
    ...
    # в молдавском. Поэтому перебираем несколько магазинов по очереди и
    # берём первый, где что-то нашлось, — это ближайший аналог "поиска
    # по всему Apple Music", который сам API не предоставляет напрямую
    # (единого "глобального" режима у него нет, только по одной стране).
    # Список стран переопределяется переменной окружения ITUNES_COUNTRIES
    # (через запятую).
