...
def ai_powered_search(text: str) -> list[dict]:
    ...
    подсказки, но решение оставить полностью за Claude):
      1. Claude по тексту гостя предлагает 1-3 варианта "исполнитель -
         название" (_ask_claude). Ничего не предложил — используется сырой
         текст гостя как единственный вариант (см. точечный фикс ниже),
         каталоги и второй запрос к Claude всё равно вызываются.
      2. Deezer + MusicBrainz опрашиваются по этим вариантам как подсказка
    ...
    if not guesses:
        ...
        # второй запрос к Claude) вообще не вызывался — хотя именно второй
        # запрос по замыслу (см. DECISION_PROMPT и решение пользователя
        # 2026-09-17) уполномочен отвечать из собственного знания Claude,
        # даже если каталоги ничего не нашли. Первый запрос мог вернуть []
        # просто потому, что не смог сформулировать структурированный
        # "исполнитель - название" — это не значит, что второй, финальный
        # запрос тоже откажется. Поэтому вместо немедленного отказа сырой
        # текст гостя используется как единственный вариант и всё равно
        # проходит оставшийся пайплайн.
        logger.info(
