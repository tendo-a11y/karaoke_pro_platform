...
def get_club_qr_links(admin, club_id: int) -> dict:
    """
    Замена старого venue_qr_code (ОДИН QR на клуб — Telegram deep-link,
    выбор стола происходил внутри бота). Guest App принимает table_no
    только из URL, экрана выбора стола внутри самого приложения нет
    (см. App.jsx::parseLinkParams) — поэтому здесь один QR НА КАЖДЫЙ стол
    клуба (решение согласовано при планировании блока), от 1 до
    club.table_count включительно. Рендер самого QR-изображения — на
    фронтенде Admin App, бэкенд отдаёт только ссылки.
    """
    club = _resolve_visible_club(admin, club_id)
    base_url = current_app.config["GUEST_APP_URL"].rstrip("/")
    table_count = club.table_count or 0

    links = [
        {"table_no": n, "url": f"{base_url}/?club_id={club.club_id}&table_no={n}"}
        for n in range(1, table_count + 1)
    ]
    return {"club_id": club.club_id, "table_count": table_count, "links": links}
