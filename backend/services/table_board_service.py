...
Как только место освобождается (KJ отклонил стоящий на нём заказ, либо
песня ушла из живой очереди VirtualDJ — оба случая уже переводят Order в
STATUS_REJECTED существующим кодом, см. reject_order() и "потерянные"
заказы в get_kj_queue_view(), services/vdj_service.py), следующий по
времени "невидимый" заказ того же стола сам занимает освободившееся место
— окно каждый раз считается заново по текущим данным, никакого отдельного
шага "продвижения в очереди" не требуется. Он появляется у KJ как обычная
заявка, ждущая решения (запрос пользователя: "всегда с моим решением новый
заказ") — confirm_order/reject_order не менялись, они и так уже требуют
явного действия KJ по каждому заказу.
"""
