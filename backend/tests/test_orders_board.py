...
def test_completing_queued_order_promotes_next_waiting_order(client, db, club, kj):
    """
    2026-09-18: подтверждение заказа больше не ставит песню в VirtualDJ
    само (см. services/vdj_service.py::confirm_order) — единственный способ
    освободить занятое место теперь явная кнопка "Готово"
    (complete_order()), а не сверка с живой очередью VirtualDJ, которая тут
    больше ни при чём (нет vdj_item_id).
    """
    club.table_count = 1
    ...
    o1 = _make_order(db, club.club_id, 111, table_no=1, offset_seconds=0)
    o2 = _make_order(db, club.club_id, 222, table_no=1, offset_seconds=1, song_title="Ждёт своей очереди")

    confirm_resp = client.put(f"/api/kj/order/{o1.id}/confirm", headers=_headers(kj["token"]))
    assert confirm_resp.status_code == 200
    assert confirm_resp.get_json()["data"]["status"] == "queued"

    resp = client.get(f"/api/kj/orders-board/{club.club_id}", headers=_headers(kj["token"]))
    slots = resp.get_json()["data"][0]["slots"]
    assert [s["order_id"] for s in slots] == [o1.id]

    complete_resp = client.put(f"/api/kj/order/{o1.id}/complete", headers=_headers(kj["token"]))
    assert complete_resp.status_code == 200
    assert complete_resp.get_json()["data"]["status"] == "completed"

    resp = client.get(f"/api/kj/orders-board/{club.club_id}", headers=_headers(kj["token"]))
    slots = resp.get_json()["data"][0]["slots"]
    assert [s["order_id"] for s in slots] == [o2.id]
