...
Тесты соответствуют сценариям из ТЗ, раздел 42 "ТЕСТИРОВАНИЕ":
  ...
  2. Подтверждение             -> test_confirm_moves_order_to_vdj_queue
  3. Двойное нажатие           -> test_double_confirm_returns_409 /
                                   test_concurrent_double_confirm_adds_song_once
...
from models import STATUS_ERROR, STATUS_PENDING, STATUS_QUEUED, STATUS_REJECTED, Order
...
def test_confirm_moves_order_to_vdj_queue(client, club, kj, bot_headers):
    ...
    assert any(item["vdj_item_id"] == data["vdj_item_id"] for item in queue)
...
def test_concurrent_double_confirm_adds_song_once(app, club, kj, bot_headers, client, monkeypatch):
    from vdj.mock_client import MockVirtualDJClient
    slow_vdj = MockVirtualDJClient()
    real_add = slow_vdj.add_to_queue
    add_call_count = []  # список, а не int — чтобы не возиться с nonlocal из потоков

    def slow_add(*args, **kwargs):
        add_call_count.append(1)  # фиксируем сам факт вызова add_to_queue
        time.sleep(0.15)  # расширяет окно гонки, чтобы оба потока точно пересеклись
        return real_add(*args, **kwargs)

    monkeypatch.setattr(slow_vdj, "add_to_queue", slow_add)
    monkeypatch.setattr("services.vdj_service.get_vdj_client", lambda *a, **kw: slow_vdj)
    ...
    assert len(slow_vdj.get_queue()) == 1, "песня должна быть добавлена в VirtualDJ ровно один раз"
    # Прямая проверка требования заказчика: проигравшие 409-запросы не должны
    # доходить до vdj.add_to_queue() вообще — не просто "в очереди 1 песня по
    # факту", а именно "функция вызвана ровно один раз".
    assert len(add_call_count) == 1, (
        f"add_to_queue() должен вызываться ровно один раз (только победителем гонки), "
        f"а вызван {len(add_call_count)} раз(а) — значит проигравшие запросы "
        f"не останавливаются на 409 и всё равно идут в VirtualDJ"
    )


# --- Тест 4: ошибка VirtualDJ ---
def test_vdj_failure_sets_error_status(app, client, club, kj, bot_headers, monkeypatch):
    with app.app_context():
        get_vdj_client().force_failure(True)
    try:
        order_id = create_order(client, bot_headers, club_id=club.club_id).get_json()["data"]["id"]
        resp = client.put(f"/api/kj/order/{order_id}/confirm", headers=kj["headers"])
        assert resp.status_code == 502
        assert resp.get_json()["error"] == "VDJ_UNAVAILABLE"
        assert resp.get_json()["order"]["status"] == STATUS_ERROR
    finally:
        with app.app_context():
            get_vdj_client().force_failure(False)
