...
проверка ранга превратилась в "нельзя всегда". Актуальная матрица:
...
# --- STATUS_QUEUED -> можно (ИЗМЕНЕНО 2026-09-20, см. докстринг файла) ---
def test_replace_allowed_when_queued(client, db, club, app):
    ...
    прежняя проверка ранга здесь больше не применяется вообще: замена
    разрешена весь срок, пока заказ ждёт исполнения (до "Готово")."""
    session = _guest_session(client, club.club_id)
    order = _create_order(client, session["token"])
...
def test_list_orders_exposes_can_replace_flag(client, db, club, app):
    """ИЗМЕНЕНО 2026-09-20: queued тоже даёт can_replace=True (см. докстринг
    файла) — раньше здесь проверялось rank 1 -> False, но с отвязкой
    confirm_order() от VirtualDJ эта ветка стала недостижимой."""
    session = _guest_session(client, club.club_id)
