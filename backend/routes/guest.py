...
from models import ChatMessage, Club, Order, Service
...
def link_google():
    ...
    if table_no is None:
        ...
        if not isinstance(table_no, int):
            return api_error(400, "VALIDATION_ERROR", "table_no должен быть числом")
...
def list_my_orders():
    ...
    """
    query = Order.query.filter_by(club_id=g.club_id, telegram_user_id=g.guest_id)
    days = request.args.get("days", type=int)
    if days is not None:
