...
def replace_order(order_id):
    """
    Заявка на замену песни в уже созданном заказе (старое: handlers/client.py
    ...
    Одобрить/Отклонить)"): раньше этот эндпоинт менял песню в заказе сразу.
    Теперь он только создаёт заявку (OrderChangeRequest) — реальная замена
    происходит, когда KJ одобрит её в KJ Panel (см.
    vdj_service.approve_order_change_request, routes/kj.py). Ответ — сама
    заявка (со status="pending"), а не изменённый заказ, — заказ пока не
    менялся вообще.

    club_id/guest_id берутся ТОЛЬКО из проверенного токена (g.*), как и в
    create_order выше — владелец заказа проверяется в
    vdj_service.request_order_replace, HTTP-слой лишь маппит её outcome на
    коды ответа.
    """
    payload = request.get_json(silent=True) or {}
    song_title = payload.get("song_title")
    ...
    change_request, outcome = vdj_service.request_order_replace(
    ...
    return api_ok(change_request.to_dict(), status_code=201)
...
#
# ИЗМЕНЕНО 2026-09-20 (решение пользователя "Нужно одобрение KJ (запрос →
# Одобрить/Отклонить)"): раньше этот эндпоинт отменял заказ немедленно и
# безусловно. Теперь он только создаёт заявку — см. комментарий у
# replace_order выше, здесь ровно та же смена модели.
@bp.post("/order/<int:order_id>/cancel")
@require_guest
def cancel_order(order_id):
    """Гость подаёт заявку на отмену своего заказа, пока он ещё не сыгран
    (см. vdj_service.request_order_cancel) — владелец заказа проверяется
    там же, HTTP-слой только маппит outcome на коды ответа. Реальная
    отмена происходит только после одобрения KJ."""
    change_request, outcome = vdj_service.request_order_cancel(order_id, g.guest_id, g.club_id)
...
def list_my_orders():
    ...
    pending_change_request (ДОБАВЛЕНО 2026-09-20, решение пользователя
    "Нужно одобрение KJ"): неразобранная заявка гостя на отмену/замену
    этого заказа, если она есть — {"id", "kind"} или null. Фронтенду это
    нужно здесь же (а не отдельным round-trip на каждую строку), чтобы
    вместо кнопок "🔁 Заменить"/"❌ Отменить" показать "ждите решения
    ведущего" — та же причина, что и у can_replace выше.
    """
    ...
    # Один запрос на все неразобранные заявки этих заказов сразу, а не по
    # одной на каждую строку (тот же принцип, что и у waiting_positions/
    # queue_positions выше — они тоже посчитаны один раз для всего списка).
    order_ids = [order.id for order in orders]
    pending_requests_by_order = {}
    if order_ids:
        pending_change_requests = (
            OrderChangeRequest.query
            .filter(
                OrderChangeRequest.order_id.in_(order_ids),
                OrderChangeRequest.status == STATUS_ORDER_CHANGE_PENDING,
            )
            .all()
        )
        for change_request in pending_change_requests:
            pending_requests_by_order[change_request.order_id] = {
    ...
    for order in orders:
        ...
        data["pending_change_request"] = pending_requests_by_order.get(order.id)
        result.append(data)
