...
# 2026-09-22, третье решение пользователя (после случая "Вниз по небесной
# лестнице" -> ложно приписано Талькову): второй запрос к Claude раньше мог
# отвечать "из собственного знания" вообще без проверки — именно так и
# появилась выдуманная песня. Пользователь прямо попросил: "только честно
# всё" и "реализуй тот же самый поиск [что и в чате]" — там, где Claude
# перед ответом реально проверяет себя веб-поиском по настоящим источникам
# (тексты песен, дискографии), а не отвечает по ассоциации. Поэтому здесь
# подключён тот же официальный инструмент веб-поиска Anthropic
# (web_search_20250305) — тексты песен и дискографии не лицензированный
# музыкальный каталог вроде iTunes/Apple Music (которые пользователь прямо
# запретил 2026-09-17), поэтому запрет на них не нарушается: ищем
# подтверждение факта "такая песня существует и её написал/исполнил X", а
# не сверяемся с наличием в конкретном коммерческом магазине.
...
результаты ниже даны как ПОДСКАЗКА, а не как источник истины.

У тебя есть инструмент веб-поиска — ОБЯЗАТЕЛЬНО используй его, чтобы
проверить догадку по реальным источникам (тексты песен, дискографии,
музыкальные сайты), прежде чем дать окончательный ответ, особенно если:
  - каталоги ничего не нашли или нашли что-то явно не то;
  - "первоначальный вариант" ниже — это на самом деле необработанный текст
    гостя (обрывок фразы или строчка из песни), а не пара
    "исполнитель - название";
  - ты не уверен(а) на 100% в исполнителе или точном названии.

Если поиск подтвердил песню — отвечай уверенно, даже если это редкий
региональный репертуар, которого нет в Deezer/MusicBrainz (именно ради
него это и разрешено). Если поиск ничего не подтвердил и ты по-прежнему
не уверен(а) — верни пустой список. НЕ называй правдоподобно звучащего
исполнителя или название только по стилистической ассоциации без
подтверждения поиском — гостю это покажут как факт, а не как догадку.
...
Когда закончишь искать и проверять, в последнем сообщении верни СТРОГО
JSON без пояснений, в формате:

{{"results": [{{"title": "название", "artist": "исполнитель или null"}}]}}

От 0 до 3 наиболее вероятных вариантов, отсортированных по убыванию
уверенности. Если ты вообще не уверен(а) ни в одном варианте даже после
поиска — верни {{"results": []}}, не отвечай наугад."""
...
def _ask_claude_to_decide(context: str, guesses: list[str], catalog_hits: list[dict]) -> list[dict]:
    ...
    первоначальными вариантами и подсказками от Deezer/MusicBrainz, и
    теперь с доступом к официальному веб-поиску Anthropic
    (web_search_20250305), которым Claude обязан проверить догадку перед
    ответом (см. DECISION_PROMPT и решение пользователя 2026-09-22 выше).
    Решение по-прежнему остаётся полностью за Claude — просто теперь оно
    подтверждено поиском, а не выдумано по ассоциации."""
    api_key = current_app.config.get("ANTHROPIC_API_KEY")
    if not api_key:
    ...
    try:
        resp = _call(with_search=True)
    except requests.RequestException:
        logger.exception("Ошибка сети при обращении к Claude API (решение)")
        return []

    if resp.status_code != 200:
        # Модель или окружение может не поддерживать инструмент веб-поиска
        # (см. решение пользователя 2026-09-22 — фикс не должен полностью
        # ломать поиск, если сервер веб-поиска недоступен). Пробуем один
        # раз без инструмента, чтобы не терять функциональность целиком —
        # это возвращает старое (непроверенное) поведение только на время
        # сбоя, а не насовсем.
        logger.error(
            "Claude API (решение, с веб-поиском) вернул %s: %s — пробуем без веб-поиска",
            resp.status_code, resp.text,
        )
        try:
            resp = _call(with_search=False)
        except requests.RequestException:
            logger.exception("Ошибка сети при обращении к Claude API (решение, без веб-поиска)")
            return []
        if resp.status_code != 200:
            logger.error("Claude API (решение, без веб-поиска) тоже вернул %s: %s", resp.status_code, resp.text)
            return []

    try:
        data = resp.json()
        # С веб-поиском ответ — это несколько блоков (текст, вызовы
        # инструмента, результаты поиска, снова текст); окончательный JSON
        # находится в ПОСЛЕДНЕМ текстовом блоке, а не обязательно в первом.
...
def ai_powered_search(text: str) -> list[dict]:
    ...
    подсказки, но решение оставить полностью за Claude; 2026-09-22,
    третье решение — раз Claude всё равно проверяет себя веб-поиском
    (см. _ask_claude_to_decide и докстринг ниже), сырой текст гостя
    больше не отбрасывается сразу, а тоже проверяется):
      1. Claude по тексту гостя предлагает 1-3 варианта "исполнитель -
         название" (_ask_claude). Ничего не предложил — используется
         сырой текст гостя как единственный вариант (см. точечный фикс
         ниже), каталоги и второй запрос к Claude всё равно вызываются.
      2. Deezer + MusicBrainz опрашиваются по этим вариантам как подсказка
      ...
      3. Claude ещё раз, уже видя и свои первоначальные варианты, и то, что
         ...
         (_ask_claude_to_decide) — теперь с доступом к веб-поиску, которым
         обязан проверить догадку по реальным источникам, прежде чем
         ответить, а не полагаться на "показалось похоже". Вправе ответить
         из собственного знания (подтверждённого поиском), даже если оба
         каталога ничего не нашли — так и должно быть для репертуара,
         которого в этих каталогах в принципе нет.
    """
    text = text.strip()
    ...
    if not guesses:
        # Точечный фикс 2026-09-22 (жалоба пользователя: "Ion Aldea-Teodorovici
        # Focul din vatră" даёт пустой результат). Раньше здесь сразу
        # возвращался [], и весь остальной пайплайн (Deezer/MusicBrainz +
        # второй запрос к Claude) вообще не вызывался. Теперь сырой текст
        # гостя используется как единственный вариант и всё равно проходит
        # оставшийся пайплайн — но, в отличие от первой версии этого фикса
        # (см. историю коммитов), второй запрос к Claude теперь обязан
        # проверить его веб-поиском, а не отвечать "из головы" наугад: было
        # замечено, что без этого для обрывков текста песни (а не
        # "исполнитель - название") Claude может уверенно назвать
        # несуществующую песню/не того исполнителя (реальный случай:
        # "Вниз по небесной лестнице" -> ложно приписано Талькову вместо
        # настоящего автора, Меладзе).
        logger.info(
            "AI-поиск: текст гостя %r -> Claude не предложил вариантов, пробуем сырой текст гостя как запрос",
            text,
        )
        guesses = [text]
